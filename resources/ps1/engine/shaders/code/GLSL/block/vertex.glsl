...
void main() {
    ...
    light.rgb += nat;
    vec4 vertpos = projection * view * vec4(fragPos, 1.0);
    gl_Position = vertpos;
    gl_Position.xyz /= vertpos.w;
    gl_Position.x = floor(gl_Position.x * 320.0) / 320.0;
    gl_Position.y = floor(gl_Position.y * 240.0) / 240.0;
    gl_Position.z = floor(gl_Position.z * 32768.0) / 32768.0;
    gl_Position.xyz *= vertpos.w;
}
