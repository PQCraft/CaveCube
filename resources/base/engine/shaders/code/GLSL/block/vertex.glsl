...
// [4 bits: R][4 bits: G][4 bits: B][4 bits: natural light multiplier][8 bits: texture X][8 bits: texture Y]
layout (location = 2) in uint data3;
// [16 bits: texture offset][7 bits: animation offset][7 bits: animation divisor][1 bit: texture X + 1][1 bit: texture Y + 1]
uniform mat4 view;
...
void main() {
    ...
    texCoord.x = (float(((data2 >> 8) & uint(255)) + ((data3 >> 1) & uint(1)))) / 16.0;
    texCoord.y = (float((data2 & uint(255)) + (data3 & uint(1)))) / 16.0;
    fragPos += vec3(ccoord.x, 0.0, ccoord.y) * 16.0;
    texOffset = float(((data3 >> 16) & uint(65535)) + ((aniMult / ((data3 >> 2) & uint(127))) % ((data3 >> 9) & uint(127))));
