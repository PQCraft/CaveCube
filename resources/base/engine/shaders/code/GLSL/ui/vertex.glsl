...
// text:    [1 bit: 1][8 bits: reserved][8 bits: reserved (16-bit charset maybe)][8 bits: text char][8 bits: z]
...
void main() {
    ...
    if ((elemType = ((data2 >> 31) & uint(1))) == uint(1)) {
        ...
        texCoord.x = float((data4 >> 24) & uint(255)) / float((data4 >> 8) & uint(255));
        texCoord.y = float((data4 >> 16) & uint(255)) / float(data4 & uint(255));
